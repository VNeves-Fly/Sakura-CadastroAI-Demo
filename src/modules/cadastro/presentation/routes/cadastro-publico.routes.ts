...
export async function createAgenciaRoute(request: Request) {
  try {
    ...
    const socios = await Promise.all(
      parsedMeta.data.socios.map(async (socioMetaItem, index) => {
        ...
        return {
          ...
          rgNumero: socioMetaItem.rgNumero,
          rgOrgaoEmissor: socioMetaItem.rgOrgaoEmissor,
          nacionalidade: socioMetaItem.nacionalidade,
          administrativo: socioMetaItem.administrativo,
    ...
    const agencia = await cadastroPublicoController.finalizarCadastro({
      ...
      nomeFantasia: parsedMeta.data.nomeFantasia,
