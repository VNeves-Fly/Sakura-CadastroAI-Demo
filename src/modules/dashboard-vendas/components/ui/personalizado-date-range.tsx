...
// mesmo desenho do print de referência do usuário (2026-08-19).
...
// Painel de um mês dentro do popover — cada dia é um botão; visual de
// intervalo (barra rosa clara contínua + extremidades em pill sólido)
// calculado célula a célula a partir de inicio/fim do rascunho atual.
function PainelMes({
  mes,
  hoje,
  inicio,
...
// Filtro "Personalizado" — mesmo contrato de props de antes (string
// dd/mm/aaaa via onDataInicialChange/onDataFinalChange), só que agora com
// calendário completo em vez de campo de texto livre (pedido do usuário,
// 2026-08-19, print de referência). Usado nos 4 lugares com filtro de
// período: Resumo do dia, Top 10 Agências, Top 10 Fornecedores
// (dashboard-vendas) e o hero de vendas do Executivo (atribuicoes) — troca
// aqui dentro já vale pros 4 automaticamente.
export function PersonalizadoDateRange({
  ...
  // Clique em dia: primeiro clique começa um intervalo novo (início = fim
  // = dia clicado); segundo clique fecha o intervalo (se for antes do
  // início, inverte). Terceiro clique começa outro intervalo, e assim por
  // diante — mesmo comportamento de qualquer date-range picker padrão.
  ...
  function aplicarAtalho(dias: number) {
    setRascunhoInicio(subDays(hoje, dias - 1));
    setRascunhoFim(hoje);
    setAindaNaoEscolheuFim(false);
    setMesEsquerda(subMonths(startOfMonth(hoje), 1));
  }

  function aplicar() {
    onDataInicialChange(format(rascunhoInicio, FORMATO));
    onDataFinalChange(format(rascunhoFim, FORMATO));
    setAberto(false);
  }
  ...
  const rotuloAtual =
    dataInicial && dataFinal ? `${dataInicial} - ${dataFinal}` : "Selecionar período";
        ...
        <div className="flex items-center justify-center gap-2 sm:gap-3">
          {ATALHOS.map((atalho) => (
            <button
              ...
              className="border-primary/30 text-primary hover:bg-primary/5 rounded-full border px-3 py-1.5 text-xs font-semibold whitespace-nowrap"
            >
              {atalho.label}
            </button>
          ))}
