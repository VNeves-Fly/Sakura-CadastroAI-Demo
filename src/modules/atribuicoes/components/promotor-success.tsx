...
import type { CreatedPromotorResult } from "@/modules/atribuicoes/types/promotor-crud.types";

interface PromotorSuccessProps {
  result: CreatedPromotorResult;
  onDismiss: () => void;
}

export function PromotorSuccess({ result, onDismiss }: PromotorSuccessProps) {
  ...
  return (
    <div className="border-primary/30 bg-primary/5 relative flex flex-col gap-2 rounded-[1.5rem] border p-5">
      ...
      <p className="text-foreground pr-6 text-sm font-semibold">
        Executivo {result.promotor.nome} salvo com sucesso.
      </p>
