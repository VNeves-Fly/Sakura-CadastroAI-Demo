...
export function SwipeSwitch({ checked, onChange, id }: SwipeSwitchProps) {
  return (
    <label
      htmlFor={id}
      className="bg-destructive/70 has-[:checked]:bg-success relative inline-flex h-8 w-14 shrink-0 cursor-pointer items-center rounded-full transition-colors duration-300"
    >
      <input
        id={id}
        type="checkbox"
